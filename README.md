...
本脚本目前已精准适配以下 **6 款** 骁龙 8 Gen3平台设备：
...
* **机型目录对齐：** 脚本运行后会根据你选择的机型自动调用对应文件夹（如 `Redmik80`、`Xiaomi14ultra` 等）内的特定配置文件。
* **Windows 一键直达：** 专为 Windows 平台设计，直接运行 `8g3-unlock.bat` 即可，无需配置冗余的 ADB/Fastboot 环境。
...
如果你在上述 5 款机型上遇到“未识别设备”或脚本报错，请提交 **Issue** 并附上报错截图，我会针对该机型文件夹内的逻辑进行修复。
